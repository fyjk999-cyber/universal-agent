...
| CHAPTER 4 | Travel Multi-Source | **PARTIAL**（多源结构就绪） | 4.1 Skyscanner hardening ✅（唯一验证过的 Live 源）；**CH4 起步：FR-060 HTTP Adapter、第二 Flight 源 ctrip_http（`adapters/ctrip/`）、真实价格 API Kiwi Tequila（`adapters/kiwi/`，本机实测认证语义正确，待用户 key）、Hotel 源 booking_http（`adapters/booking/`）均已实现并测试（本地 HTTP + 真实端点管线 10 项）**；**剩余**：注册 UA_KIWI_KEY 后真实联调、真实端点联调（UA_CTRIP_ENDPOINT/UA_BOOKING_ENDPOINT）、第三 Flight 源、cross-source Entity Resolution（FR-071） |
...
| CHAPTER 7 | Additional Domains | **PARTIAL**（Railway 已 Live） | Railway：**12306 真实余票源已接入（无 key，实测 上海→杭州东 20 条，`adapters/railway/`）**，Raw+normalize+entity_key+Live search 达成（FR-110~115 部分）；Ecommerce/Food 仍仅 Raw+normalize+entity_key；每域缺完整 Score/Source/Verify/Watch/Notification |
