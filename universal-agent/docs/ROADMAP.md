...
| CHAPTER 4 | Travel Multi-Source | **PARTIAL**（多源结构就绪） | 4.1 Skyscanner hardening ✅（唯一验证过的 Live 源）；**CH4 起步：FR-060 HTTP Adapter 实现、第二 Flight 源 ctrip_http（`adapters/ctrip/`，SkillProtocol+失败隔离）、Hotel 源 booking_http（`adapters/booking/`）均已实现并测试（本地 HTTP 全链路 5 项）**；**剩余**：真实端点联调（UA_CTRIP_ENDPOINT/UA_BOOKING_ENDPOINT + key）、第三 Flight 源、cross-source Entity Resolution（FR-071） |
